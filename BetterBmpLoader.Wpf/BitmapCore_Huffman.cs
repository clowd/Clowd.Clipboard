...
namespace BetterBmpLoader
{
    internal unsafe partial class BitmapCore
    {
        private static void ReadHuffmanG31D(ref BITMAP_READ_DETAILS info, byte* sourceBufferStart, byte* destBufferStart)
        {
            ...
            string GetCodeString(int cw, int numbits)
            {
                var b = Convert.ToString(cw, 2);
                while (b.Length < numbits) b = "0" + b;
                return b;
            }

            void DebugWriteLine(int line)
            {
                debugBuilder.AppendLine($"------------ LINE {line} ------------");
            }

            void DebugWriteInstruction(G31DFaxCode fc, bool white, int pos)
            {
                var ins = GetCodeString(fc.code, fc.bitLength);
                while (ins.Length < 16) ins = ins + " ";
                debugBuilder.AppendLine($"{ins}{(white ? "white" : "black")}    rl={fc.runLength}    x_pos={pos}/{width}");
            }

            void DebugWriteRemainingAndLaunchNotepad(Exception e)
            {
                debugBuilder.AppendLine("------------ END ------------");
                debugBuilder.Append(GetCodeString(codeWord, bitsAvailable));
                for (; dataAvailable > 0; dataAvailable--)
                {
                    debugBuilder.Append(GetCodeString(*sourcePtr, 8));
                    sourcePtr++;
                }
                debugBuilder.AppendLine();
                debugBuilder.AppendLine("------------ EXCEPTION ------------");
                debugBuilder.AppendLine(e.ToString());

                const string debugFile = "BitmapCoreHuffman1DErrorLog.txt";
                System.IO.File.WriteAllText(debugFile, debugBuilder.ToString());
                System.Diagnostics.Process.Start("notepad.exe", debugFile);
            }
#endif
            ...
            //    return codeWord >> (bitsAvailable - n);
            //}
            ...
            //// the data stream sometimes begins with a white EOL code, lets detect and strip it.
            //if (GetBits(12) == 1) ClearBits(12);
    ...
    internal class CCITTHuffmanG31D
    {
        ...
        internal static readonly short[] faxWhiteCodes =
        {
            8, 0x35, 0,  /* 0011 0101 */
            6, 0x7, 1,  /* 0001 11 */
            4, 0x7, 2,  /* 0111 */
        ...
        internal static readonly short[] faxBlackCodes =
        {
        ...
        public static IEnumerable<G31DFaxCode> FromArray(short[] arr)
        {
